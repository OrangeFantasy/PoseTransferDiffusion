...
if __name__ == "__main__":
    ...
    vae_path = r"E:/_Project\stable-diffusion-webui/models/VAE/vae-ft-mse-840000-ema-pruned.ckpt"
    # ckpt = torch.load(vae_path)
    # vae_model.load_state_dict(ckpt)
    vae_model.init_from_ckpt(vae_path)
    ...
    pose_path = r"E:\_Project\_Dataset\In-shop Clothes Retrieval Benchmark\_pose\WOMEN\Pants\id_00000887\02_1_front.jpg"
    image = Image.open(image_path)
    image = vf.resize(image, [256, 256])

    x = vf.to_tensor(image).unsqueeze(0).cuda()
    # x = torch.rand([1, 3, 256, 256]).cuda()
    z = vae_model.encode(x).sample()
